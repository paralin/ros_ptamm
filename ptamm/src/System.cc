      ...
      nh_("vslam"), image_nh_(""), first_frame_(true)
{
  ...
  pub_info_ = nh_.advertise<ptamm_com::ptamm_info> ("info", 1);
...
void System::imageCallback(const sensor_msgs::ImageConstPtr & img)
{
  ...
  if (varParams.MotionModelSource == ptamm::PtamParams_MM_IMU)
  {
    sensor_msgs::Imu imu;
...
void System::publishPoseAndInfo(const std_msgs::Header & header)
{
  ...
  if (frame_id == "")
  {
    ROS_WARN_ONCE("camera frame id not set, will set it to \"ptamm\"");
    frame_id = "ptamm";
  }
  ...
  if (mpTracker->getTrackingQuality() && mpMap->IsGood())
  {
    ...
    TooN::Matrix<3, 3, double> r_ptamm = pose.get_rotation().get_matrix();
    TooN::Vector<3, double> t_ptamm =  pose.get_translation();

    tf::StampedTransform transform_ptamm(tf::Transform(tf::Matrix3x3(r_ptamm(0, 0), r_ptamm(0, 1), r_ptamm(0, 2), r_ptamm(1, 0), r_ptamm(1, 1), r_ptamm(1, 2), r_ptamm(2, 0), r_ptamm(2, 1), r_ptamm(2, 2))
    , tf::Vector3(t_ptamm[0] / scale, t_ptamm[1] / scale, t_ptamm[2] / scale)), header.stamp, frame_id, PtamParameters::fixparams().parent_frame);
    ...
    if (pub_pose_.getNumSubscribers() > 0 || pub_pose_world_.getNumSubscribers() > 0)
    {
      ...
      const tf::Quaternion & q_tf_ptamm = transform_ptamm.getRotation();
      const tf::Vector3 & t_tf_ptamm = transform_ptamm.getOrigin();
      msg_pose->pose.pose.orientation.w = q_tf_ptamm.w();
      msg_pose->pose.pose.orientation.x = q_tf_ptamm.x();
      msg_pose->pose.pose.orientation.y = q_tf_ptamm.y();
      msg_pose->pose.pose.orientation.z = q_tf_ptamm.z();
      msg_pose->pose.pose.position.x = t_tf_ptamm.x();
      msg_pose->pose.pose.position.y = t_tf_ptamm.y();
      msg_pose->pose.pose.position.z = t_tf_ptamm.z();
    ...
    {
      ptamm_com::ptamm_infoPtr msg_info(new ptamm_com::ptamm_info);
...
//Weiss{
bool System::pointcloudservice(ptamm_com::PointCloudRequest & req, ptamm_com::PointCloudResponse & resp)
{
...
bool System::keyframesservice(ptamm_com::KeyFrame_srvRequest & req, ptamm_com::KeyFrame_srvResponse & resp)
{
