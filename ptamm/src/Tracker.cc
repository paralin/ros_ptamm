...
void Tracker::TrackFrame(Image<CVD::byte> &imFrame, bool bDraw)
{
  ...
  //Weiss{
  const ptamm::PtamParamsConfig& pPars = PtamParameters::varparams();
...
void Tracker::ApplyMotionModel()
{
  ...
  if(mbUseSBIInit && vp.MotionModelSource == ptamm::PtamParams_MM_CONSTANT)
  {
    v6Velocity.slice<3,3>() = mv6SBIRot.slice<3,3>();
    v6Velocity[0] = 0.0;
    v6Velocity[1] = 0.0;
  }
  else if(vp.MotionModelSource == ptamm::PtamParams_MM_IMU)
  {
    //
